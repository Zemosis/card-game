    ...
    <div className="min-h-screen bg-gradient-to-br from-indigo-900 via-purple-900 to-pink-900 flex items-center justify-center p-6">
      <div className="max-w-4xl w-full bg-gray-900/80 backdrop-blur rounded-2xl shadow-2xl overflow-hidden flex flex-col md:flex-row border border-white/10">
        ...
        <div className="w-full md:w-1/3 p-6 border-b md:border-b-0 md:border-r border-gray-700 bg-gray-800/50">
          <button onClick={() => navigate('/')} className="text-gray-400 hover:text-white mb-6 flex items-center gap-2 text-sm">
            ← Back to Menu
          </button>
          ...
          {/* Create Section */}
          <div className="mb-8">
            <h3 className="text-sm font-bold text-blue-400 uppercase mb-3">Create Lobby</h3>
            <input 
              type="text" 
          ...
          <div>
            <h3 className="text-sm font-bold text-green-400 uppercase mb-3">Join via Code</h3>
            <div className="flex gap-2">
              <input 
                type="text" 
          ...
          <div className="flex-1 overflow-y-auto space-y-3 pr-2 custom-scrollbar">
            {publicLobbies.length === 0 ? (
              ...
              publicLobbies.map(lobby => (
                <div key={lobby.id} className="bg-gray-800 hover:bg-gray-750 border border-gray-700 rounded-lg p-4 flex items-center justify-between transition-colors">
                  <div>
                    <div className="font-bold text-white text-lg">{lobby.name}</div>
                    <div className="text-sm text-gray-400">Host: <span className="text-gray-300">{lobby.host}</span></div>
                  </div>

                  <div className="flex items-center gap-4">
                    <div className="text-right">
                      <div className="text-sm font-bold text-gray-300">Players</div>
                      <div className={`text-lg font-mono ${lobby.current === lobby.max ? 'text-red-400' : 'text-green-400'}`}>
