...
const LobbySelection = () => {
  ...
  const handleQuickStart = () => {
    ...
    // Generate a local lobby ID and go straight to the game as host.
    // GameThirteen detects the socket is offline and initialises locally.
    ...
    navigate("/game-13", {
      state: {
        ...
        myPlayerIndex: 0, // pin player index — eliminates socket-ID lookup entirely
      },
    });
  };
    ...
    <div className="min-h-screen bg-gradient-to-br from-indigo-900 via-purple-900 to-pink-900 flex items-center justify-center p-6">
      <div className="max-w-4xl w-full bg-gray-900/80 backdrop-blur rounded-2xl shadow-2xl overflow-hidden flex flex-col md:flex-row border border-white/10">
        ...
        <div className="w-full md:w-1/3 p-6 border-b md:border-b-0 md:border-r border-gray-700 bg-gray-800/50">
          ...
          {/* Quick Start — offline solo play */}
          <div className="mt-6 pt-6 border-t border-gray-600">
            <h3 className="text-sm font-bold text-amber-400 uppercase mb-1">
              Quick Start
            </h3>
            <p className="text-xs text-gray-400 mb-3">
              Play vs AI locally — no server needed.
            </p>
            <button
              onClick={handleQuickStart}
              className="w-full bg-amber-500 hover:bg-amber-400 active:scale-95 text-white py-2.5 rounded-lg text-sm font-bold transition-all duration-150"
            >
              ▶ Solo vs AI
            </button>
          </div>
